/** Composant non utilisé dans l'app actuelle. Conservé pour référence. Voir ui/_archived/README.md */
...
void main(){gl_Position=position;}`;
    ...
    init() {
      if (!this.gl || !this.program || !this.isValid) return;
      const gl = this.gl;
  ...
  const resize = () => {
    ...
    const dpr = Math.min(MAX_DPR, Math.max(1, 0.5 * (window.devicePixelRatio || 1)));
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (!(rendererRef.current as any).isValid) {
        console.warn('WebGL renderer initialization failed, using fallback background');
      ...
      if (testResult === null) {
        ...
        console.error('Shader test failed:', testResult);
      }
